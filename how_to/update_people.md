...
1. For best uniformity, crop the image to a verticallly-oriented 4x5 ratio. Images with other ratios will show up fine, but will change the relative height of the person's profile listing. You can crop using [Preview on a mac], [the Photos app], [somehow else on Windows], or [online using this website].
2. Upload the image to the `assets/img/` folder. For best practice, use a descriptive file name. 

### Creating a biography file

1. Create a new file in the `_pages` folder following the naming convention `about_lastname.md`. 
...
```
  - align: right
    image: prof_pic.jpg
    alt: Albert Einstein gazes into the camera in a black and white portrait.
...
`align` indicates which side the profile picture will be shown on. For style, this alternates with every profile, which must be set manually (all profiles below must be changed, if one is added above). <br>`image` points to the file name for the profile picture that has been added to `assets/img/`. Only the file name, not the path, is needed. <br> `alt` provides alt text for the image, which will be shown if an image fails to load and read for those using a screen reader. <br> `content` points to the file in `_pages/` containing the biography. <br> `category` indicates which role the profile should be categorized in, using one of three variables, either `*pi` for primary investigators, `*stud` for grad students and postdocs, or `*staff` for staff. <br> `image_circular` is a toggle for whether the image will be cropped to a circular (oval) shape when the website is compiled. No need to change this line. The first profile defines a variable `&all_circular false` that is used in all the other profiles so that this is controlled together. <br> `more_info` I will probably delete.
...
5. No need to change the text after `image_circular`.
...
- In `_pages/`, delete the `about_lastname.md` file that contains their biography.
