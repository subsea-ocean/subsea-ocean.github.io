...
3. Update the `title`, which will be the whole announcement if `inline` is true, and the `date`. 
    - For clarity, if the announcement is inline and you want to include a link, do not make the entire line of text a hyperlink so readers can distinguish between inline and longer-form announcements.
